...
/* ============================================================================
   组件：精致滑块（🛡️ 防重叠优化：更合理的宽高下限与自适应折行）
============================================================================ */
const Slider = ({ label, value, onChange, min, max, step = 1, unit = '' }) => (
  <div className="bg-slate-800/40 border border-slate-700/50 p-4 sm:p-5 rounded-2xl hover:border-cyan-500/40 hover:bg-slate-800/60 transition-all duration-300 flex flex-col justify-center gap-3">
    <div className="flex flex-row justify-between items-center gap-2">
      <span className="text-slate-300 font-medium text-sm sm:text-base truncate">
        {label}
      </span>
      <div className="bg-cyan-950/60 px-3 py-1.5 rounded-lg border border-cyan-500/30 shrink-0 inline-flex items-baseline gap-1 shadow-inner min-w-[72px] justify-center">
    ...
    <div className="relative flex items-center w-full mt-1">
      <input
        ...
        className="w-full h-1.5 bg-slate-700/80 rounded-lg appearance-none cursor-pointer accent-cyan-400 hover:accent-cyan-300 focus:outline-none focus:ring-2 focus:ring-cyan-500/30 transition-all"
      />
    </div>
  </div>
);
...
/* ============================================================================
   SVG: 内接法 - 原始完整电路 (文字已全部居中对齐，杜绝重叠)
============================================================================ */
const InnerOriginalSVG = ({ E, r, ra }) => (
  <svg viewBox="0 0 300 210" className="w-full h-auto max-w-[340px]">
    {/* 等效黑盒框 */}
    <rect x="30" y="115" width="240" height="70" rx="10" fill="rgba(244,63,94,0.04)" stroke="#f43f5e" strokeWidth="2" strokeDasharray="5 4" />
    <text x="150" y="132" fill="#f87171" fontSize="12" fontWeight="bold" textAnchor="middle">等效黑盒 (真实电源 + 电流表)</text>
    ...
    <text x="150" y="45" fill="#38bdf8" fontSize="14" fontWeight="bold" textAnchor="middle">V</text>
    ...
    <text x="150" y="105" fill="#94a3b8" fontSize="14" fontWeight="bold" textAnchor="middle">R</text>
    ...
    <text x="150" y="200" fill="#4ade80" fontSize="13" fontFamily="monospace" fontWeight="bold" textAnchor="middle">E={E}V, r={r}Ω</text>
    ...
    <text x="240" y="135" fill="#f43f5e" fontSize="14" fontWeight="bold" textAnchor="middle">A</text>
...
const InnerEquivalentSVG = ({ E_eq, r_eq }) => (
  <svg viewBox="0 0 300 210" className="w-full h-auto max-w-[340px]">
    ...
    <text x="150" y="146" fill="#34d399" fontSize="12" fontWeight="bold" textAnchor="middle">理想等效电源（内阻已合并）</text>
    ...
    <text x="150" y="200" fill="#10b981" fontSize="13" fontFamily="monospace" fontWeight="bold" textAnchor="middle">E'={E_eq}V, r'={r_eq}Ω</text>
  </svg>
);
...
const OuterOriginalSVG = ({ E, r, rv }) => (
  <svg viewBox="0 0 300 210" className="w-full h-auto max-w-[340px]">
    ...
    <text x="150" y="35" fill="#38bdf8" fontSize="12" fontWeight="bold" textAnchor="middle">等效黑盒 (真实电源 + 电压表)</text>
    ...
    <circle cx="150" cy="40" r="16" fill="#0f172a" stroke="#38bdf8" strokeWidth="2.5" />
    <text x="150" y="45" fill="#38bdf8" fontSize="14" fontWeight="bold" textAnchor="middle">V</text>
    <text x="175" y="48" fill="#38bdf8" fontSize="11" fontFamily="monospace">
    ...
    <text x="105" y="105" fill="#94a3b8" fontSize="14" fontWeight="bold" textAnchor="middle">R</text>
    ...
    <text x="200" y="105" fill="#f43f5e" fontSize="14" fontWeight="bold" textAnchor="middle">A</text>
...
const OuterEquivalentSVG = ({ E_eq, r_eq }) => (
  <svg viewBox="0 0 300 210" className="w-full h-auto max-w-[340px]">
    <rect x="30" y="130" width="240" height="55" rx="10" fill="rgba(16,185,129,0.04)" stroke="#10b981" strokeWidth="2" strokeDasharray="5 4" />
    <text x="150" y="146" fill="#34d399" fontSize="12" fontWeight="bold" textAnchor="middle">理想等效电源（电压表已吸收）</text>
...
const TransformArrow = () => (
  <div className="flex flex-col items-center justify-center text-gray-500 py-6 lg:py-0 lg:px-6 shrink-0">
    <span className="text-xs font-bold mb-2 text-cyan-400 whitespace-nowrap tracking-wide bg-cyan-950/40 px-3 py-1 rounded-full border border-cyan-500/20 shadow-sm">戴维南等效</span>
...
const TheveninView = ({ stats }) => (
  <div className="w-full flex flex-col gap-10 xl:gap-14">
    {/* 内接法区块 */}
    <div className="bg-slate-900/40 border border-slate-700/50 backdrop-blur-md rounded-3xl p-6 md:p-8 xl:p-10 shadow-xl shadow-rose-500/5">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-5">
        <h3 className="text-xl md:text-2xl font-black text-rose-400 flex items-center gap-3">
          <span className="bg-rose-500/15 px-4 py-1.5 rounded-xl text-sm border border-rose-500/30 backdrop-blur-sm shadow-inner">内接法</span>
          等效黑盒构造
        </h3>
        {/* 测量数据牌（自适应折行） */}
        <div className="flex flex-wrap gap-3 sm:gap-4 font-mono text-sm w-full lg:w-auto">
          <div className="bg-slate-950/80 px-5 py-2.5 rounded-xl border border-rose-500/20 flex items-center shrink-0 shadow-md">
      ...
      <div className="w-full bg-[#03050a] rounded-2xl p-6 lg:p-10 border border-white/5 flex flex-col lg:flex-row items-center justify-evenly gap-4 md:gap-8 shadow-inner">
    ...
    {/* 外接法区块 */}
    <div className="bg-slate-900/40 border border-slate-700/50 backdrop-blur-md rounded-3xl p-6 md:p-8 xl:p-10 shadow-xl shadow-sky-500/5">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-5">
        <h3 className="text-xl md:text-2xl font-black text-sky-400 flex items-center gap-3">
          <span className="bg-sky-500/15 px-4 py-1.5 rounded-xl text-sm border border-sky-500/30 backdrop-blur-sm shadow-inner">外接法</span>
          等效黑盒构造
        </h3>
        <div className="flex flex-wrap gap-3 sm:gap-4 font-mono text-sm w-full lg:w-auto">
          <div className="bg-slate-950/80 px-5 py-2.5 rounded-xl border border-sky-500/20 flex items-center shrink-0 shadow-md">
            <span className="text-slate-400 mr-2 flex items-baseline">E<span className="text-[10px] ml-0.5">测</span></span>
...
const ErrorGraphView = ({ stats }) => {
  ...
  return (
    <div className="w-full flex flex-col gap-10">
      {/* 图像面板 */}
      <div className="relative bg-[#020617] border border-slate-700/50 rounded-3xl overflow-hidden shadow-2xl w-full">
        {/* 顶部图例栏 */}
        <div className="flex flex-wrap items-center gap-x-6 gap-y-3 px-6 md:px-8 py-5 bg-slate-900/80 backdrop-blur-md border-b border-slate-800 text-xs md:text-sm font-mono">
          <div className="flex items-center gap-2 shrink-0">
            <div className="w-6 h-0 border-t-[3px] border-emerald-400 border-dashed"></div>
            <span className="text-emerald-400 font-semibold tracking-wide">真实电源</span>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <div className="w-6 h-1 bg-rose-500 rounded-full"></div>
            <span className="text-rose-400 font-semibold tracking-wide">内接法（E不变）</span>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <div className="w-6 h-1 bg-sky-500 rounded-full"></div>
            <span className="text-sky-400 font-semibold tracking-wide">外接法（整体平移）</span>
          </div>
        </div>
        ...
        <div className="p-4 md:p-8">
          <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto drop-shadow-[0_0_20px_rgba(0,0,0,0.8)]">
            ...
            {/* 坐标轴 */}
            <line x1={padding} y1={padding - 15} x2={padding} y2={height - padding} stroke="#94a3b8" strokeWidth="2" strokeLinecap="round" markerStart="url(#arrow)" />
            <line x1={padding} y1={height - padding} x2={width - padding + 20} y2={height - padding} stroke="#94a3b8" strokeWidth="2" strokeLinecap="round" markerEnd="url(#arrow)" />
            
            <text x={width - padding + 25} y={height - padding + 5} fontSize="14" fill="#cbd5e1" fontFamily="monospace" fontWeight="bold">I (A)</text>
            <text x={padding} y={padding - 25} fontSize="14" fill="#cbd5e1" textAnchor="middle" fontFamily="monospace" fontWeight="bold">U (V)</text>
            
            {/* 曲线绘制 */}
            <line
              x1={mapX(0)} y1={mapY(stats.E)}
              x2={mapX(stats.E / stats.r)} y2={mapY(0)}
              stroke="#4ade80" strokeWidth="3" strokeDasharray="8 6" filter="url(#glow)" strokeLinecap="round"
            />
            <line
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        <div className="bg-gradient-to-br from-rose-500/10 to-rose-900/5 border border-rose-500/20 rounded-3xl p-6 md:p-8 relative overflow-hidden backdrop-blur-sm shadow-lg">
          <div className="absolute top-0 right-0 w-40 h-40 bg-rose-500/15 blur-3xl rounded-full"></div>
          <div className="text-rose-400 text-xs font-bold mb-6 uppercase tracking-[0.2em] relative z-10 border-l-2 border-rose-400 pl-3">内接法 · 系统误差</div>
          <div className="space-y-5 relative z-10">
            <div className="flex justify-between items-end border-b border-rose-500/10 pb-3">
              <span className="text-slate-300 text-sm font-medium">电动势 E</span>
              <span className="text-rose-300 font-mono font-bold text-xl drop-shadow-sm">0.0 %</span>
            </div>
            <div className="flex justify-between items-end">
              <span className="text-slate-300 text-sm font-medium">内阻 r</span>
              <span className="text-rose-500 font-mono font-black text-4xl tracking-tighter drop-shadow-md">
                {formatError(stats.err_r_inner)}<span className="text-2xl ml-1">%</span>
              </span>
            </div>
          </div>
        </div>
        
        <div className="bg-gradient-to-br from-sky-500/10 to-sky-900/5 border border-sky-500/20 rounded-3xl p-6 md:p-8 relative overflow-hidden backdrop-blur-sm shadow-lg">
          <div className="absolute top-0 right-0 w-40 h-40 bg-sky-500/15 blur-3xl rounded-full"></div>
          <div className="text-sky-400 text-xs font-bold mb-6 uppercase tracking-[0.2em] relative z-10 border-l-2 border-sky-400 pl-3">外接法 · 系统误差</div>
          <div className="space-y-5 relative z-10">
            <div className="flex justify-between items-end border-b border-sky-500/10 pb-3">
              <span className="text-slate-300 text-sm font-medium">电动势 E</span>
              <span className="text-sky-400 font-mono font-bold text-xl drop-shadow-sm">
                {formatError(stats.err_E_outer)} %
              </span>
            </div>
            <div className="flex justify-between items-end">
              <span className="text-slate-300 text-sm font-medium">内阻 r</span>
              <span className="text-sky-400 font-mono font-black text-4xl tracking-tighter drop-shadow-md">
                {formatError(stats.err_r_outer)}<span className="text-2xl ml-1">%</span>
              </span>
            </div>
          </div>
        </div>
...
export default function EmfInternalResistanceLab() {
  ...
  return (
    <div className="relative p-5 sm:p-8 md:p-10 xl:p-12 bg-gradient-to-br from-[#020617] via-[#0a0f1c] to-[#020617] rounded-[2rem] md:rounded-[3rem] shadow-[0_0_80px_rgba(0,0,0,0.7)] border border-slate-800/60 text-gray-200 my-10 overflow-hidden max-w-[1400px] mx-auto font-sans w-full">
      {/* 沉浸式环境光 */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[80%] h-[40%] bg-cyan-500/10 rounded-full blur-[150px] pointer-events-none"></div>
      <div className="absolute bottom-0 left-0 w-1/2 h-1/3 bg-rose-500/10 rounded-full blur-[140px] pointer-events-none"></div>
      
      <div className="relative z-10 w-full mb-12">
        <h3 className="text-2xl md:text-3xl font-black text-white mb-8 flex items-center justify-center md:justify-start gap-3">
          <span className="text-cyan-400 text-3xl md:text-4xl drop-shadow-[0_0_15px_rgba(34,211,238,0.8)]">⚡</span>
          全局参数中控台
        </h3>
        {/* 控制区：响应式栅格 */}
      ...
      {/* 丝滑视图切换 Tabs */}
      <div className="flex justify-center mb-10 relative z-10 w-full">
        <div className="flex flex-col sm:flex-row p-1.5 bg-slate-900/60 backdrop-blur-2xl border border-white/10 rounded-2xl sm:rounded-full w-full sm:w-auto gap-2 shadow-2xl">
          <button
            ...
            className={`px-6 py-3 rounded-xl sm:rounded-full text-sm font-bold transition-all duration-300 w-full sm:w-auto ${
              activeTab === 'thevenin'
                ? 'bg-cyan-500 text-slate-950 shadow-[0_0_25px_rgba(34,211,238,0.5)]'
          ...
          <button
            ...
            className={`px-6 py-3 rounded-xl sm:rounded-full text-sm font-bold transition-all duration-300 w-full sm:w-auto ${
              activeTab === 'error'
                ? 'bg-purple-500 text-white shadow-[0_0_25px_rgba(168,85,247,0.5)]'
                : 'text-slate-400 hover:text-white hover:bg-white/10'
            }`}
          >
            U‑I 图像与相对误差
